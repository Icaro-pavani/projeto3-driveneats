...
const myPlate = document.getElementsByClassName('prato'); 
const myDrink = document.getElementsByClassName('bebida');
const myDessert = document.getElementsByClassName('sobremesa');

// Abaixo os botòes do HTML são alocas em variáveis
const botao = document.querySelector('.checar-pedido');
const botaoCancelar = document.querySelector('.cancelar');
const botaoPedir = document.querySelector('.pedir');

// Desabilita o botão para confirmar o pedido na tela principal
botao.disabled = true;

// Definição das variáveis globais
let pos = null;    //posições dos dados dentro da tabela da confirmação do pedido
let total = null;  //Total do pedido
let nome = null;   //Aloca o nome quando pedido no pormpt  
let endereco = null;   //Aloca o endereço pedido no pormpt
...
function selecionar(secao) {
    // ativa como se fosse onclick para cada item
    for (let item of secao) {
...
function mandarMensagem() {
    ...
    // abre uma nova janela para o whats com a mensagem
...
function checarPedido() {
    ...
    // Number() funciona como um parseInt ou ParseFloat
